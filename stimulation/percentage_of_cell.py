...
class StimPercentageOfCell(Stim):
    ...
    def get_stim_mask(self, label_image: np.ndarray, metadata: dict = None, img: np.array = None) -> np.ndarray:
        light_map = np.zeros_like(label_image)
        props = skimage.measure.regionprops(label_image)
        percentage_of_stim = metadata.get('treatment', {}).get('stim_cell_percentage', 0.3)
        ...
        try: 
            ...
            for prop in props: 
                ...
                cutoff_mask = np.fromfunction(lambda i, j: self.above_line(j, i, x3, y3, x2, y2), np.shape(label_image), dtype=int)

                frame_labeled_expanded = skimage.segmentation.expand_labels(single_label, 5)
