        ...
        try:
            for exp_time in df_acquire["time"].unique():
                ...
                for index, row in current_time_df.iterrows():
                    ...
                    if timestep > 0:
                        fov.tracks = fov.tracks_queue.get(
                            block=True
                        )  # wait max 10s for tracks
                    ...
                    for i, channel in enumerate(channels):
                        ...
                        acquisition_event = useq.MDAEvent(
                            ...
                            },  # the channel presets we want to acquire
                        ...
                        self._queue.put(acquisition_event)
                    if stim:
                        metadata_dict["img_type"] = (
                            ImgType.IMG_STIM
                        )  # change the img_type and channels, rest stays the same
                        metadata_dict["last_channel"] = True
                        metadata_dict["channel"] = stim_profile["channel"]
                        zero_stim_prop = PropertyTuple(
                            stim_profile["device_name"],
                            stim_profile["property_name"],
                            0,
                        )

                        if (
                            stim_exposure == 0
                        ):  # not really a good solution, need to improve
                            ...
                            stimulation_event = useq.MDAEvent(
                                ...
                                channel={
                                    "config": stim_profile["channel"],
                                    "group": self._current_group,
                                },  # the channel presets we want to acquire
                                metadata=metadata_dict,  # (custom) metadata that is attatched to the event/image
                                x_pos=row["fov_object"].pos[
                                    0
                                ],  # only one pos for all channels
                            ...
                            if self._dmd != None:
                                stim_mask = fov.stim_mask_queue.get(
                                    block=True
